...
type
  ...
  public
    ...
    procedure ValidarCPFCNPJ(FCPFCNPJ:String;FID:Int64);
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          jsonObject := jsonArray.Items[1] as TJSONObject;
          if (jsonObject.GetValue('valor').Value <> EmptyStr) then
          begin
            Q.SQL.Add('WHERE '+jsonObject.GetValue('nome').Value+' = :'+jsonObject.GetValue('nome').Value);
            if (UpperCase(jsonObject.GetValue('nome').Value) = 'PAIS') then
              Q.ParamByName(jsonObject.GetValue('nome').Value).AsLargeInt := RetornaIDNomeLigacao(jsonObject.GetValue('valor').Value,'PAISES')
            else
              Q.ParamByName(jsonObject.GetValue('nome').Value).AsLargeInt := RetornaIDNomeLigacao(jsonObject.GetValue('valor').Value,'ESTADOS');
          end;
        end;
      end;
    finally
...
begin
  ...
  try
    QCon.SQL.Add('SELECT ID '+
                 'FROM '+FTabela+' ');
    if (FTabela = 'ESTADOS') then
      QCon.SQL.Add('WHERE UPPER(SIGLA) = :NOME')
    else
      QCon.SQL.Add('WHERE UPPER(NOME) = :NOME');
...
begin
  ...
  try
    ...
    try
      ...
      ValidarCPFCNPJ(MemTable.FieldByName('CPFCNPJ').AsString,MemTable.FieldByName('ID').AsLargeInt);
...
procedure TConsulta.ValidarCPFCNPJ(FCPFCNPJ: String; FID: Int64);
begin
  var Q := NewSQuery;
  try
    Q.SQL.Add('SELECT COUNT(ID) AS QTD '+
              'FROM CLIENTES '+
              'WHERE CPFCNPJ = :DADO '+
              'AND ID NOT IN (:ID)');
    Q.ParamByName('DADO').AsString := FCPFCNPJ;
    Q.ParamByName('ID').AsLargeInt := FID;
    Q.Open();

    if (Q.FieldByName('QTD').AsLargeInt > 0) then
